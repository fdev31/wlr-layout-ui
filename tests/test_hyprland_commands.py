...
class TestOldSyntax:
    """Tests for the legacy keyword monitor syntax."""
    ...
    def test_full_command(self):
        ...
        cmd = _make_command_hyprland_old([screen], [rect])
        assert "hyprctl --batch" in cmd
        assert 'keyword monitor DP-1,1920x1080@60.00Hz,0x0,1.500000,transform,1' in cmd

    def test_disabled_monitor(self):
        screen = _make_screen(active=False)
        rect = _make_rect()
        cmd = _make_command_hyprland_old([screen], [rect])
        assert 'keyword monitor DP-1,disable' in cmd
    ...
    def test_multiple_monitors(self):
        ...
        cmd = _make_command_hyprland_old([s1, s2], [r1, r2])
        assert 'keyword monitor DP-1,1920x1080@60.00Hz,0x0,1.000000,transform,0' in cmd
        assert 'keyword monitor HDMI-A-1,2560x1440@144.00Hz,1920x0,2.000000,transform,2' in cmd
...
class TestLuaSyntax:
    """Tests for the new Lua hl.monitor() syntax."""
    ...
    def test_full_command(self):
        ...
        r1 = _make_rect(0, 0)
        r2 = _make_rect(1920, 0)
        cmd = _make_command_hyprland_lua([s1, s2], [r1, r2])
    ...
    def test_disabled_monitor(self):
        screen = _make_screen(active=False)
        rect = _make_rect()
        cmd = _make_command_hyprland_lua([screen], [rect])
        assert 'hl.monitor({output="DP-1", disable=true})' in cmd
